...
public class NoteItemPane : Box {
  ...
  private void handle_key_events( Widget w ) {
  ...
  private Widget add_image_item( NoteItemImage item, int pos = -1 ) {
    ...
    image.add_controller( image_focus );

    if( item.uri == "" ) {
      image_dialog( item, image );
    } else {
    ...
    image_focus.enter.connect(() => {
      set_current_item( Utils.get_child_index( _content, box ), null );
      box.add_css_class( "active-item" );
    });
  ...
  private Widget add_uml_item( NoteItemUML item, int pos = -1 ) {
    ...
    image_click.pressed.connect((n_press, x, y) => {
      if( n_press == 1 ) {
        ...
        set_current_item( Utils.get_child_index( _content, stack ), text );
        stack.visible_child_name = "input";
        text.grab_focus();
      }
    });
    ...
    image_focus.enter.connect(() => {
      set_current_item( Utils.get_child_index( _content, stack ), text );
      stack.add_css_class( "active-item" );
    });

    image_focus.leave.connect(() => {
      stack.remove_css_class( "active-item" );
    });
