  ...
  */
  ...
  public static void insert_image_text( GtkSource.View view, TextBuffer buffer ) {
    ...
    if( buffer.get_selection_bounds( out start, out end ) ) {
      ...
      try {
        if( Uri.is_valid( selected, UriFlags.NONE ) && Utils.is_uri_supported_image( selected ) ) {
          var text = "![](" + selected + ")";
          buffer.begin_user_action();
          buffer.delete( ref start, ref end );
          buffer.insert( ref start, text, text.length );
      ...
      if( clipboard.get_formats().contain_gtype( Type.STRING ) ) {
        clipboard.read_text_async.begin( null, (obj, res) => {
          try {
            var text = clipboard.read_text_async.end( res );
            if( (FileUtils.test( text, FileTest.EXISTS ) || Uri.is_valid( text, UriFlags.NONE )) && Utils.is_uri_supported_image( text ) && insert_snippet( view, buffer, "%md-image-clip%", text ) ) {
              return;
            } else {
