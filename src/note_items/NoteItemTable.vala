...
public class NoteItemTable : NoteItem {
  ...
  //-------------------------------------------------------------
  // Default constructor
  public NoteItemTable( NoteItemRow row, int columns = 0, int rows = 0 ) {
    base( row, NoteItemType.TABLE );
    _columns = new Array<NoteItemTableColumn>();
    _rows    = new ListStore( typeof( NoteItemTableRow ) );
    for( int i=0; i<columns; i++ ) {
      var table_column = new NoteItemTableColumn( i );
      _columns.append_val( table_column );
      table_column.changed.connect( handle_change );
    }
    for( int i=0; i<rows; i++ ) {
      var table_row = new NoteItemTableRow( columns );
  ...
  public NoteItemTable.from_xml( NoteItemRow row, Xml.Node* node ) {
    base( row, NoteItemType.TABLE );
    _columns = new Array<NoteItemTableColumn>();
    _rows    = new ListStore( typeof( NoteItemTableRow ) );
    load( node );
